...
def adjust_manually(self, image_data, x=None):
	...
	color_image = cv2.cvtColor(image.astype(np.uint8),cv2.COLOR_BAYER_BG2BGR)
	...
	bayer = mt.repmat(np.array([['red', 'green'], ['green', 'blue']]), int(image_data.mem_info.height / 2),
	                  int(image_data.mem_info.width / 2))
